    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (!eDao.isThereAnyReportableExam(examSessionId)) {
                writeHttpResponse(response, HttpServletResponse.SC_BAD_REQUEST, "text/plain", null);
                return;
            }
            var reportId = "" + eDao.report(examSessionId);
            writeHttpResponse(response, HttpServletResponse.SC_OK, "text/plain", reportId);
            return;
