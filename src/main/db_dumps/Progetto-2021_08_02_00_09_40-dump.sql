...
CREATE TABLE `exams` (
  ...
  CONSTRAINT `gradeLaudeCompatibility` CHECK ((((`grade` is null) and (`laude` is null)) or ((`grade` <> 30) and (`laude` = false)) or (`grade` = 30))),
  CONSTRAINT `resultGradeCompatibility` CHECK (((`result` is null) or ((`result` = _utf8mb4'missing') and (`grade` is null)) or ((`result` = _utf8mb4'passed') and (`grade` between 18 and 30)) or (((`result` = _utf8mb4'rejected') or (`result` = _utf8mb4'sent back')) and (`grade` between 0 and 17)))),
  CONSTRAINT `statusResultCompatibility` CHECK ((((`status` = _utf8mb4'not inserted yet') and (`result` is null)) or (`result` is not null)))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
